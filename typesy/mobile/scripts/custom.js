...
function calculateEstimator(){// estimator

		var noEmployees = parseFloat($(document.getElementById("numberEmployees").value));
		var hoursPerDay = parseFloat($(document.getElementById("hoursPerDay").value));
		var hourlyRate  = parseFloat($(document.getElementById("hourlyRate").value));

		var hoursLost  = (hoursPerDay * 0.33) * noEmployees;
		var dailyLost  = hoursLost * hourlyRate;
		var annualLost = dailyLost * 261;
		
		var dhr = $(document.getElementById("dailyHoursResult"));
		var dlr = $(document.getElementById("dailyLostResult"));
		var alr = $(document.getElementById("annualLostResult"));
		dhr.html( Number(hoursLost).toFixed(0).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') + "*");
		dlr.html ("$" + Number(dailyLost).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') + "*");
		alr.html ("$" + Number(annualLost).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') + "*");
...
observer.observe();	
const ssImage = document.querySelector('.load-ss')
// ... trigger the load of a image before it appears on the viewport
observer.triggerLoad(ssImage);		
