IF EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'[dbo].[form_submission]') AND OBJECTPROPERTY(id, N'ISUSERTABLE') = 1)
DROP TABLE [dbo].[form_submission]
GO
...
IF EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'[dbo].[respondent]') AND OBJECTPROPERTY(id, N'ISUSERTABLE') = 1)
DROP TABLE [dbo].[respondent]
GO

IF EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'[dbo].[form]') AND OBJECTPROPERTY(id, N'ISUSERTABLE') = 1)
DROP TABLE [dbo].[form]
GO
...
IF EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'[dbo].[organization]') AND OBJECTPROPERTY(id, N'ISUSERTABLE') = 1)
...
IF NOT EXISTS (SELECT * FROM sysindexes WHERE id = Object_id('dbo.form_submission') AND NAME = 'form_submission_uuid_index')
  CREATE INDEX form_submission_uuid_index ON dbo.form_submission (uuid);
go

IF NOT EXISTS (select * from sysindexes where id=object_id('dbo.respondent') and name='respondent_last_name_index')
CREATE INDEX respondent_last_name_index ON dbo.respondent (last_name);
go
​
IF NOT EXISTS (select * from sysindexes where id=object_id('dbo.respondent') and name='respondent_first_name_last_name_index')
CREATE INDEX respondent_first_name_last_name_index ON dbo.respondent (first_name, last_name);
go
​
IF NOT EXISTS (select * from sysindexes where id=object_id('dbo.respondent') and name='respondent_email_index')
CREATE INDEX respondent_email_index ON dbo.respondent (email);
go